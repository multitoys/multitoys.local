<?php
    ...
    if ($may_order) {
        ...
        if (array_key_exists('conc', $_POST)) {
            ...
            $all_res = '<ul><li>Результаты поиска - ';
        } else {
        ...
        $count_results = (int)mysql_num_rows($query);

        if ($count_results > 0 && $count_results < 500) {
            if (array_key_exists('conc', $_POST)) {
                $all_res .= '<b style=\'color:red;\'>'.$count_results.'</b></li>';
            }
            echo $all_res;
...
//                    $code = stripAll($_POST['code']);
            ...
            echo '</ul>';
        } elseif ($count_results > 500) {
            echo '
            Найдено более 
            <b style=\'color:red;\'>'.$count_results.'. </b>
            Попробуйте изменить запрос...
            </li>
            <ul><li><label>Мы ничего не нашли... </label> :(' . $close . '</li></ul>';
        } else {
