...
//            $_XXX = 'var';
            ...
            $local_delivery_cost = isset($_POST['market_export_local_delivery_cost'])? (float)str_replace(',', '.', $_POST['market_export_local_delivery_cost']) :false;
...
//            $_XXX = 'var';
            ...
            for (; $iter < $proCount; $iter += $chunk_size) {
                ...
                $sql = "select productID, product_code, ".LanguagesManager::sql_prepareField('name')." AS name, Price, categoryID, default_picture".$dsc_q.", in_stock, slug, eproduct_filename, skidka, code_1c, ukraine, min_order_amount, zakaz".($local_delivery_cost_enabled?', free_shipping, shipping_freight':'')." from ".PRODUCTS_TABLE."
...
//                $picture_url = (MOD_REWRITE_SUPPORT&&false)?$store_url.'products_pictures/':BASE_URL.URL_PRODUCTS_PICTURES.'/';
                ...
                while ($product = db_fetch_row($q)) {
                    ...
                    fputs($f, "					<offer available=\"".(($product['zakaz'])?'':'true')."\" id=\"".$product["productID"]."\">\n");
