...
//			"classID"			- tax class ID
    function GetProduct($productID)
    {
		...
		';
            ...
            $product['new'] = _getNewIDs($product['productID']);
...
//								"option_value"		- value
    ...
    function _getNewIDs($productID = '') 
    {
        $and_where = '';
        if ($productID) {
            $and_where = ' AND productID='.$productID;
        }
        $q = "SELECT productID FROM SC_category_product WHERE categoryID=66666 $and_where";
        $r = mysql_query($q) or die();
        
        while ($ids = mysql_fetch_row($r)) {
            $values[$ids[0]] = 1;
        }
//        var_dump($values);
        return $values;
...
//					"extraParametrsTemplate"
    function prdSearchProductByTemplate($callBackParam, &$count_row, $navigatorParams = '')
    {
        ...
        if (isset($callBackParam['search_simple'])) {
            ...
            if (!count($callBackParam['search_simple'])) { //empty array
				...
				LOWER('.LanguagesManager::sql_prepareField('brief_description').') LIKE ?'.$search_name.'
                 OR  LOWER(brand) LIKE ?'.$search_name.') ';
        ...
        while ($_Product = db_fetch_assoc($Result)) {
            ...
            $_Product['new'] = $new_ids[$_Product['productID']];
            $Products[] = $_Product;
            $Counter++;
        }
