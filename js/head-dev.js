...
$(document).ready(function () {
     ...
     */
    function actionCategory(afterLoading) {
        if (afterLoading == 'load') {
            try {
                mainScroll = baron({
                    root: '.scroll-pane1',
                    scroller: '#content',
            ...
            // $("#columns").height((mainColumnsHeight > columnsHeight)?mainColumnsHeight:columnsHeight);
            // $(".cpt_product_lists").height((mainColumnsHeight > columnsHeight)?mainColumnsHeight:columnsHeight);
        }
    }
    function actionDefault(afterLoading) {
        if (afterLoading == 'load') {
            try {
                listScroll = baron({
            ...
            // $("#columns").height((mainColumnsHeight > columnsHeight)?mainColumnsHeight:columnsHeight);
            var cH = $("#columns").height;
            
            $(".cpt_product_lists").height((mainColumnsHeight > cH($("#center").height()))?mainColumnsHeight:cH($("#center").height()));
        }
    }
    function actionCart(afterLoading) {
        if (afterLoading == 'resize') {
            $("#columns").height(1);
        } else if (afterLoading != 'load' ) {
            $("#columns").height($("#center").height());
        }
    }
    function initAction(cuttedPath, afterLoading) {
        switch (cuttedPath) {
            case 'cart':
                actionCart(afterLoading);
                break;
            case 'category':
                actionCategory(afterLoading);
            default:
                actionDefault(afterLoading);
        }
    }
    ...
    function resizeCatalog() {
        ...
        initAction(cuttedPath[1], "resize");
        setTimeout(realResizeCatalog, 500);
    }
    function realResizeCatalog() {
        headerHeight = $("#header").height() + $(".header").height() + 8;
        ...
        columnsHeight =  $(document).height() - headerHeight;

        initAction(cuttedPath[1], "realresize");
